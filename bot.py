...
MAIN_ADDRESS       = 'TSw9PuQCTp3kHcPVni2uKo8c5PJiHvxupp'  # ваш основной кошелёк
MAIN_PRIVATE_KEY   = os.getenv('MAIN_PRIVATE_KEY', '')      # приватный ключ нового TronLink кошелька
USDT_CONTRACT      = 'TR7NHqjeKQxGTCi8q8ZY4pL8otSzgjLj6t'
CODES_FILE         = os.getenv('DATA_DIR', '/data') + '/codes.json'
CHECK_INTERVAL     = 30
...
# Tron клиент
tron_client = Tron(HTTPProvider(api_key=TRONGRID_KEY))
...
def send_trx_for_fee(to_address):
    ...
    if not MAIN_PRIVATE_KEY:
        logger.warning('MAIN_PRIVATE_KEY не задан — не могу отправить TRX для комиссии')
        return False
    try:
        priv = PrivateKey(bytes.fromhex(MAIN_PRIVATE_KEY))
        sender = priv.public_key.to_base58check_address()
        amount_sun = TRX_FOR_FEE * 1_000_000  # TRX → SUN
        txn = (
...
def forward_usdt(private_key_hex, from_address, amount_usdt):
    ...
    try:
        ...
        # Оставляем 0.1 USDT на случай погрешностей
...
async def monitor_wallets(bot):
    ...
    while True:
        try:
            ...
            min_ts = int(time.time() * 1000) - 3600000  # последний час

            for code, wallet_info in wallets.items():
                address = wallet_info['address']
                private_key = wallet_info['private_key']

                txs = get_transactions(address, min_ts)
                for tx in txs:
                    ...
                    # Обновляем прогресс с новым балансом
