...
class APIHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if p.path == '/progress':
            ...
            if code:
                db = load_db()
                db.setdefault('progress', {})[code] = {'states': states, 'progs': progs, 'bal': bal, 'ts': int(time.time())}
                if bal:
        ...
        if p.path == '/load':
            ...
            self.send_json(200, {'ok': True, 'states': prog.get('states',''), 'progs': prog.get('progs',''), 'bal': bal})
            return
