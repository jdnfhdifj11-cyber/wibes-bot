...
MAIN_ADDRESS       = 'TSw9PuQCTp3kHcPVni2uKo8c5PJiHvxupp'
MAIN_PRIVATE_KEY   = os.getenv('MAIN_PRIVATE_KEY', '')
USDT_CONTRACT      = 'TR7NHqjeKQxGTCi8q8ZY4pL8otSzgjLj6t'
CODES_FILE         = 'codes.json'
DB_CHANNEL_ID      = -1003991235770
DB_MESSAGE_ID      = None
CHECK_INTERVAL     = 30
TRX_PER_USDT       = 3  # 1 USDT = 3 TRX (курс пополнения)
TRX_PRICE_USD      = 0.34  # актуальный курс TRX в USD (обновляется каждые 5 мин)
...
# ── БД (файл + Telegram канал как резервная копия) ──

_db_cache = None
_db_message_id = None

def _tg_api(method, data):
    try:
    ...
    if os.path.exists(CODES_FILE):
        try:
            with open(CODES_FILE, 'r', encoding='utf-8') as f:
                data = json.load(f)
            _db_cache = data
            _db_message_id = data.get('_msg_id')
            return data
        except Exception as e:
            logger.error(f'File load error: {e}')
    # Возвращаем пустую базу
    empty = {"active":[],"used":{},"balances":{},"wallets":{},
             "withdrawals":[],"transactions":[],"progress":{},"_msg_id":None}
    _db_cache = empty
    return empty
...
def save_db(data):
    ...
    # Резервная копия в Telegram канал
    try:
        text = json.dumps(data, ensure_ascii=False)[:3900]
        if _db_message_id:
            res = _tg_api('editMessageText', {
                'chat_id': DB_CHANNEL_ID,
                'message_id': _db_message_id,
                'text': f'```\n{text}\n```',
                'parse_mode': 'Markdown'
            })
            if not res or not res.get('ok'):
                _db_message_id = None
        if not _db_message_id:
            res = _tg_api('sendMessage', {
                'chat_id': DB_CHANNEL_ID,
                'text': f'```\n{text}\n```',
                'parse_mode': 'Markdown'
            })
            if res and res.get('ok'):
                _db_message_id = res['result']['message_id']
                data['_msg_id'] = _db_message_id
                _db_cache = data
                with open(CODES_FILE, 'w', encoding='utf-8') as f:
                    json.dump(data, f, ensure_ascii=False, indent=2)
    except Exception as e:
        logger.error(f'TG backup error: {e}')
...
def get_trx_price():
    """Получает актуальный курс TRX/USD с Binance."""
    try:
    ...
    try:
        ...
        pass
    return TRX_PRICE_USD  # fallback к последнему известному курсу

async def update_trx_price():
    """Обновляет курс TRX каждые 5 минут."""
    global TRX_PRICE_USD
    while True:
        try:
            price = await asyncio.get_event_loop().run_in_executor(None, get_trx_price)
            if price and price > 0:
                TRX_PRICE_USD = price
                logger.info(f'TRX price updated: ${price:.4f}')
        except Exception as e:
            logger.error(f'Price update error: {e}')
        await asyncio.sleep(300)

def get_trx_transactions(address, min_timestamp=0):
    """Получает входящие TRX транзакции на адрес."""
    try:
        url = (
            f'https://api.trongrid.io/v1/accounts/{address}/transactions'
            f'?only_to=true&limit=20&min_timestamp={min_timestamp}'
        )
        req = urllib.request.Request(url, headers={
            'TRON-PRO-API-KEY': TRONGRID_KEY,
            'Accept': 'application/json'
        ...
        with urllib.request.urlopen(req, timeout=10) as resp:
            ...
            for tx in data.get('data', []):
                # Фильтруем только TransferContract (обычная отправка TRX)
                contracts = tx.get('raw_data', {}).get('contract', [])
                for c in contracts:
                    if c.get('type') == 'TransferContract':
                        val = c.get('parameter', {}).get('value', {})
                        if val.get('to_address') and val.get('amount'):
                            txs.append({
                                'transaction_id': tx.get('txID', ''),
                                'amount_trx': val['amount'] / 1_000_000,
                                'block_timestamp': tx.get('block_timestamp', 0)
                            })
            return txs
    except Exception as e:
        logger.error(f'TRX tx fetch error: {e}')
        return []

def forward_trx(private_key_hex, from_address, amount_trx):
    """Пересылает TRX с временного кошелька на главный."""
    if not TRONPY_OK or not tron_client:
        return False
    try:
        priv = PrivateKey(bytes.fromhex(private_key_hex))
        send_amount = int((amount_trx - 1) * 1_000_000)  # оставляем 1 TRX на комиссию
        if send_amount <= 0:
            return False
        txn = (
            tron_client.trx.transfer(from_address, MAIN_ADDRESS, send_amount)
            .build().sign(priv).broadcast()
        )
        result = txn.wait()
        logger.info(f'Forward {amount_trx} TRX from {from_address}: {result}')
        return True
    except Exception as e:
        logger.error(f'Forward TRX error: {e}')
        return False
...
async def monitor_wallets(bot):
    ...
    logger.info('🔍 Мониторинг TRX кошельков запущен')
    ...
    while True:
        try:
            ...
            for code, wallet_info in wallets.items():
                ...
                # Получаем TRX транзакции в отдельном потоке
                txs = await loop.run_in_executor(None, get_trx_transactions, address, min_ts)
                ...
                for tx in txs:
                    ...
                    amount_trx = tx.get('amount_trx', 0)
                    if amount_trx < 1:
                        continue

                    # Зачисляем TRX напрямую без конвертации
                    amount_trx = tx.get('amount_trx', 0)

                    logger.info(f'Новая транзакция для {code}: +{amount_trx} TRX')

                    db.setdefault('balances', {})[code] = round(
                        db['balances'].get(code, 0) + amount_trx, 2
                    )
                    processed.append(tx_id)
                    db['transactions'] = processed

                    # Записываем историю пополнений
                    dep_record = {
                        'amount': amount_trx,
                        'txid': tx_id,
                        'date': datetime.now().strftime('%d.%m.%Y %H:%M'),
                        'usd': round(amount_trx * TRX_PRICE_USD, 2)
                    }
                    if code in db.get('progress', {}):
                        db['progress'][code]['bal'] = db['balances'][code]
                        # Добавляем в depHistory
                        try:
                            import json as _json
                            dep_hist = _json.loads(db['progress'][code].get('depHistory', '[]'))
                            dep_hist.insert(0, dep_record)
                            db['progress'][code]['depHistory'] = _json.dumps(dep_hist)
                        except:
                            db['progress'][code]['depHistory'] = _json.dumps([dep_record])
                    else:
                        db.setdefault('progress', {})[code] = {
                            'states': '', 'progs': '', 'bal': db['balances'][code],
                            'history': '[]',
                            'depHistory': json.dumps([dep_record])
                        }
                    ...
                    await bot.send_message(
                        ...
                        text=(
                            ...
                            f"⚡ Зачислено: <b>{amount_trx:.2f} TRX</b>\n"
                            f"💵 (~${amount_trx * TRX_PRICE_USD:.2f})\n"
                            f"📍 Адрес: <code>{address}</code>\n"
                            f"🔗 TX: <code>{tx_id[:20]}...</code>\n\n"
                            f"⚡ Пересылаю TRX на ваш кошелёк..."
                        ),
                        parse_mode='HTML'
                    )

                    # Пересылаем TRX на главный кошелёк
                    def forward_task(pk=private_key, addr=address, amt=amount_trx, c=code):
                        time.sleep(5)
                        success = forward_trx(pk, addr, amt)
                        if success:
                            logger.info(f'TRX успешно переслан от {addr}')
                        else:
                            logger.error(f'Ошибка пересылки TRX от {addr}')
    ...
    def do_GET(self):
        ...
        if p.path == '/balance':
            ...
            self.send_json(200, {
                ...
                'trx_price': TRX_PRICE_USD,
                'trx_per_usdt': TRX_PER_USDT
            })
            return
...
async def cmd_sendtrx(u: Update, _):
    """Отправляет TRX на все кошельки где реально нет TRX."""
    if not is_admin(u.effective_user.id): return
    if not MAIN_PRIVATE_KEY:
    ...
    wallets = db.get('wallets', {})
    if not wallets:
        await u.message.reply_text("📭 Нет кошельков.")
        return
    await u.message.reply_text(f"⏳ Проверяю {len(wallets)} кошельков...")
    need_trx = []
    for code, w in wallets.items():
        real_trx = get_trx_balance(w['address'])
        if real_trx < 5:  # меньше 5 TRX — нужно пополнить
            need_trx.append((code, w))
    if not need_trx:
        await u.message.reply_text("✅ У всех кошельков достаточно TRX.")
        return
    await u.message.reply_text(f"⚡ Нужно пополнить {len(need_trx)} кошельков TRX...")
    sent = 0
    for code, wallet in need_trx:
        success = send_trx_for_fee(wallet['address'])
        if success:
            db['wallets'][code]['trx_sent'] = True
            sent += 1
            logger.info(f'TRX отправлен на {wallet["address"]}')
        time.sleep(3)
    save_db(db)
    await u.message.reply_text(
        f"✅ TRX отправлен на {sent}/{len(need_trx)} кошельков.\n"
        f"Через 30 сек бот автоматически перешлёт USDT на ваш кошелёк."
    )
    if not is_admin(u.effective_user.id): return
    db = load_db()
    total = sum(float(w.get('amount', 0)) for w in db.get('withdrawals', []))
...
def main():
    ...
    async def post_init(application):
        global _db_cache, _db_message_id
        try:
            logger.info('Восстанавливаю БД из Telegram канала...')
            best_data = None
            best_score = -1
            best_msg_id = None
            # Перебираем последние 20 сообщений канала
            for msg_id in range(1, 30):
                try:
                    res = await application.bot.forward_message(
                        chat_id=ADMIN_ID,
                    ...
                    if text.startswith('{'):
                        ...
                        # Считаем "вес" данных
                        score = (len(data.get('used',{})) + 
                                 len(data.get('wallets',{})) + 
                                 len(data.get('withdrawals',[])) +
                                 sum(data.get('balances',{}).values()))
                        if score > best_score:
                            best_score = score
                            best_data = data
                            best_msg_id = msg_id
            ...
            if best_data:
                _db_cache = best_data
                _db_message_id = best_msg_id
                with open(CODES_FILE, 'w') as f:
                    json.dump(best_data, f, ensure_ascii=False, indent=2)
                logger.info(f'✅ БД восстановлена (score={best_score}, msg_id={best_msg_id})')
            else:
                logger.warning('⚠️ Не удалось восстановить БД из Telegram')
        except Exception as e:
            logger.error(f'DB restore error: {e}')
        asyncio.create_task(monitor_wallets(application.bot))
        asyncio.create_task(update_trx_price())
