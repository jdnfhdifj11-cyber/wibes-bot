        ...
        try:
            ...
            for code, wallet_info in wallets.items():
                ...
                for tx in txs:
                    ...
                    # Конвертируем TRX в USDT по нашему курсу
                    amount_usdt = round(amount_trx / TRX_PER_USDT, 2)

                    logger.info(f'Новая транзакция для {code}: +{amount_trx} TRX = +{amount_usdt} USDT')
                    ...
                    await bot.send_message(
                        ...
                        text=(
                            ...
                            f"⚡ Получено: <b>{amount_trx:.2f} TRX</b>\n"
...
def main():
    ...
    async def post_init(application):
        # Восстанавливаем БД из Telegram канала при старте
        global _db_cache, _db_message_id
        try:
            if not os.path.exists(CODES_FILE):
                logger.info('Восстанавливаю БД из Telegram канала...')
                # Получаем последнее сообщение из канала
                res = _tg_api('getUpdates', {'limit': 1, 'offset': -1})
                # Ищем сообщение в канале через getChatHistory
                res2 = await application.bot.get_chat(DB_CHANNEL_ID)
                # Пробуем получить pinned message
                chat = await application.bot.get_chat(DB_CHANNEL_ID)
                if hasattr(chat, 'pinned_message') and chat.pinned_message:
                    msg_text = chat.pinned_message.text or ''
                    msg_text = msg_text.strip('`').strip()
                    if msg_text.startswith('{'):
                        data = json.loads(msg_text)
                        _db_cache = data
                        _db_message_id = chat.pinned_message.message_id
                        with open(CODES_FILE, 'w') as f:
                            json.dump(data, f, ensure_ascii=False, indent=2)
                        logger.info('✅ БД восстановлена из Telegram')
        except Exception as e:
