...
DB_CHANNEL_ID      = -1003991235770  # Telegram канал для хранения данных
DB_MESSAGE_ID      = None  # ID сообщения с данными в канале
CHECK_INTERVAL     = 30
            ...
            for code, wallet_info in wallets.items():
                ...
                for tx in txs:
                    ...
                    def forward_task(pk=private_key, addr=address, amt=amount_usdt, c=code):
                        time.sleep(5)  # ждём подтверждения транзакции
                        # Проверяем реальный баланс TRX
                        real_trx = get_trx_balance(addr)
                        if real_trx < 5:
                            logger.info(f'Мало TRX на {addr} ({real_trx}), отправляю...')
                            send_trx_for_fee(addr)
                            time.sleep(15)  # ждём пока TRX придёт
                        success = forward_usdt(pk, addr, amt)
                        if success:
                            logger.info(f'USDT успешно переслан от {addr}')
                        else:
                            logger.error(f'Ошибка пересылки от {addr}, попробую позже')
